...
const PaymentDialog = ({ totalToPay }: { totalToPay: number }) => (
  <Dialog.Root>
    <Dialog.Trigger asChild>
      <button className="mt-4 w-full rounded-lg bg-sky-700 py-2 font-mono text-white hover:bg-sky-800">
        Pay Now
      </button>
    </Dialog.Trigger>
    <Dialog.Portal>
      ...
      <Dialog.Content className="fixed left-1/2 top-1/2 w-full max-w-md -translate-x-1/2 -translate-y-1/2 bg-white p-6 shadow-lg">
        ...
        <form className="mt-4 space-y-4">
          ...
          <button
            type="submit"
            className="w-full rounded-lg bg-sky-700 py-2 font-mono text-white hover:bg-sky-800"
          >
